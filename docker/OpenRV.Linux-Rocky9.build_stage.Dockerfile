...
FROM ${BASE_IMAGE}:latest AS openrv_linux_rocky9_build_stage
# Build:
# CMAKE_GENERATOR: "Ninja" or "Unix Makefiles" (https://aur.archlinux.org/packages/openrv-git)
# export DOCKERFILE="OpenRV.Linux-Rocky9.build_stage.Dockerfile" && /usr/bin/time -f 'Commandline Args: %C\nElapsed Time: %E\nPeak Memory: %M\nExit Code: %x' docker build --file ./docker/${DOCKERFILE} --progress plain --build-arg BASE_IMAGE openstudiolandscapes/openrv_linux_rocky9_build_base_dockerfile --build-arg BUILD_ARGS="-Wno-dev" --build-arg CMAKE_GENERATOR="Ninja" --build-arg FFMPEG_NON_FREE_DECODERS_TO_ENABLE="aac;hevc" --build-arg FFMPEG_NON_FREE_ENCODERS_TO_ENABLE="aac" --tag openstudiolandscapes/$(echo ${DOCKERFILE,,} | sed -r 's/[-. ]+/_/g'):latest --tag openstudiolandscapes/$(echo ${DOCKERFILE,,} | sed -r 's/[-. ]+/_/g'):$(date +"%Y-%m-%d_%H-%M-%S") --output ./docker/tarballs . > >(tee -a ./docker/logs/${DOCKERFILE}.STDOUT.log) 2> >(tee -a ./docker/logs/${DOCKERFILE}.STDERR.log >&2) && unset DOCKERFILE
#
# Run (attached):
# Ref: https://stackoverflow.com/a/55734437/2207196
# export DOCKERFILE="OpenRV.Linux-Rocky9.build_stage.Dockerfile" && docker run --hostname $(echo ${DOCKERFILE,,} | sed -r 's/[-. ]+/_/g') --rm --name $(echo ${DOCKERFILE,,} | sed -r 's/[-. ]+/_/g') openstudiolandscapes/$(echo ${DOCKERFILE,,} | sed -r 's/[-. ]+/_/g'):latest /bin/bash -c "trap : TERM INT; sleep infinity & wait" && unset DOCKERFILE
#
# Exec bash:
# export DOCKERFILE="OpenRV.Linux-Rocky9.build_stage.Dockerfile" && docker container exec --interactive --tty $(echo ${DOCKERFILE,,} | sed -r 's/[-. ]+/_/g') /bin/bash && unset DOCKERFILE
# # Exec rv:
